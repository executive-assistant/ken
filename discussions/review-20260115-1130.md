# Executive Assistant codebase review (2026-01-15 11:53:40)
...
**Files**: `src/executive_assistant/channels/base.py`
...
**Files**: `src/executive_assistant/main.py`, `src/executive_assistant/storage/checkpoint.py`
...
**Fix Applied**:
```python
# src/executive_assistant/main.py - Changed from sync to async:
...
**File**: `src/executive_assistant/tools/python_tool.py`
...
**File**: `src/executive_assistant/tools/orchestrator_tools.py`
...
- HTTP SSE is not truly streaming because responses are buffered; yield events directly from `agent.astream` for incremental delivery. `src/executive_assistant/channels/http.py` `src/executive_assistant/channels/base.py`
- DB connection churn + scheduler race conditions: add asyncpg pools and atomic claim/lock for pending reminders/jobs to avoid duplicates. `src/executive_assistant/storage/user_registry.py` `src/executive_assistant/storage/reminder.py` `src/executive_assistant/storage/scheduled_flows.py` `src/executive_assistant/scheduler.py`
- Recurrence and cron: reminders ignore recurrence and cron parsing is limited; add croniter-based scheduling and auto-create next instances. `src/executive_assistant/scheduler.py` `src/executive_assistant/tools/reminder_tools.py` `src/executive_assistant/tools/orchestrator_tools.py`
- Config ergonomics: `TELEGRAM_BOT_TOKEN` is required even for HTTP-only runs; make it optional and validate only when Telegram is enabled. `src/executive_assistant/config/settings.py` `src/executive_assistant/channels/telegram.py`
...
## Deployment Notes
- Executive Assistant restarted with PostgreSQL checkpointer active
...
**File**: `src/executive_assistant/agent/graph.py`
...
**Files**: `src/executive_assistant/storage/file_sandbox.py`
...
### Findings
- **High**: `delegate_to_orchestrator` returns a coroutine in the async branch (sync tool cannot `await asyncio.wait_for`), which will bubble as a coroutine object or cause serialization errors. `src/executive_assistant/tools/orchestrator_tools.py`
- **High**: Thread-local fallback does not propagate `thread_id` into executor threads; `set_thread_id()` only writes the current thread id, so sync tools run in thread pools still see `None` and may fall back to global paths. `src/executive_assistant/storage/file_sandbox.py`
...
- Executive Assistant restarted with all Telegram improvements live
- No breaking changes to existing functionality
...
**File**: `src/executive_assistant/agent/nodes.py`
...
- Executive Assistant needs restart for async tool changes to take effect
...
- Replace deprecated `File.download(...)` with `File.download_to_drive(...)` and handle the returned `Path`. `src/executive_assistant/channels/telegram.py`
- Sanitize filenames before building `local_path` to prevent path traversal (e.g., `Path(file_name).name`). `src/executive_assistant/channels/telegram.py`
- Ensure `clear_thread_id()` runs in `_file_handler` and `_process_pending_jobs` to avoid leaking thread-local fallback. `src/executive_assistant/channels/telegram.py` `src/executive_assistant/scheduler.py`
- Avoid double markdown conversion by removing `_convert_markdown_to_telegram` in `handle_message` and letting `send_message` own it. `src/executive_assistant/channels/telegram.py`
